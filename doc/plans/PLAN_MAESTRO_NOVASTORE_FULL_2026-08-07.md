...
46. **Resuelta (2026-08-25): Integración de Vercel Speed Insights 2.0.0, Rediseño Dinámico de Alertas DAFO (<10%), Tarjeta de Confianza Metodológica y Auditoría CAME con NovAi:**
    - **Instalación de `@vercel/speed-insights` (versión ^2.0.0):** Se instala en `package.json` el paquete oficial `@vercel/speed-insights` (v2.0.0) y se inyecta el componente `<SpeedInsights />` en `src/app/layout.tsx` a nivel raíz de la aplicación junto a `<Analytics />` para la recolección continua de Core Web Vitals en Next.js App Router.
    - **Rediseño Contextual de Advertencias de Umbral DAFO (< 10%):** En `src/utils/investigator/domain.ts`, se actualiza la advertencia de brecha entre los dos primeros cuadrantes para que sea dinámica, cuantitativa y pedagógica: indica con precisión cuáles son los cuadrantes en disputa (ej. *DO Adaptativa vs DA Supervivencia*), el porcentaje exacto de brecha y la recomendación metodológica de formular una estrategia híbrida o afinar las calificaciones DAFO.
    - **Componente Visual de Nivel de Confianza de la Investigación (`InvestigationConfidenceCard`):** En `src/components/ui/investigation-confidence-card.tsx` e integrado en `src/views/apps/investigator/summary/index.tsx`, se implementa un indicador visual y cuantitativo del nivel de confianza metodológica (*Alta [70-100%]*, *Media [40-69%]*, *Baja [<40%]* / *No concluyente*). Mide y transparenta los 4 pilares: *Cobertura DAFO*, *Nitidez de Vector Dominante*, *Integridad de Pesos EFI/EFE* y *Mitigación de Factores Críticos en CAME*.
47. **Resuelta (2026-08-25): Recalibración del Nivel de Confianza Metodológica, Gauge Radial SVG, Acciones NovAi para Estrategia Mixta y Sincronización 1:1 de Skeletons:**
    - **Recalibración de la Regla de Confianza en `domain.ts`:** Se corrige la regla de asignación de confianza en `src/utils/investigator/domain.ts`:
      - *Confianza Alta (Verde):* Cobertura $\ge 70\%$ y brecha entre cuadrantes $\ge 10\%$.
      - *Confianza Media / Estrategia Mixta (Ámbar):* Cobertura $\ge 40\%$ (incluyendo empates técnicos donde la brecha es $< 10\%$, asignando 'media' y ámbar en lugar de penalizar indebidamente como baja cuando la muestra de cruces es robusta).
      - *Confianza Baja (Rojo):* Cobertura $< 40\%$ o ausencia de cruces evaluados.
    - **Rediseño con Radial Progress Ring SVG y Botones Interactivos NovAi en `InvestigationConfidenceCard`:** En `src/components/ui/investigation-confidence-card.tsx`, se implementa un medidor radial circular vectorial SVG con animación suave de progreso, porcentaje centrado y colores semánticos reactivos. La tarjeta fundamenta explícitamente la adopción de una postura estratégica híbrida en caso de empate técnico y agrega dos botones de acción con IA:
      - `[✨ Auditar Cobertura CAME con NovAi]`: Inicia el análisis de riesgos y debilidades no mitigadas.
      - `[✨ Justificar Estrategia Mixta con NovAi]`: Redacta la justificación ejecutiva del enfoque dual para su inclusión en el dictamen.
    - **Sincronización 1:1 de Skeletons de Carga en `summary/index.tsx`:** El bloque de skeleton en `src/views/apps/investigator/summary/index.tsx` se actualiza para reflejar con exactitud la jerarquía visual del dashboard cargado (4 KPIs superiores $\rightarrow$ columna izquierda Matriz + cuadrícula 2×2 derecha de 4 tarjetas simétricas $\rightarrow$ tarjeta ancha de Confianza Metodológica $\rightarrow$ sección de Informe Académico con tabs y contenedor de texto).

48. **Resuelta (2026-08-25): Reubicación del Plan en SidebarFooter, Botón Flotante NovAi Despejado, Persistencia Real Multi-Dispositivo de NovAi en PostgreSQL y Botón Único de Auditoría de Confianza:**
    - **Reubicación del Widget de Plan en SidebarFooter:** Se elimina el botón flotante independiente `UpgradePro.tsx`. En `src/components/layout/Sidebar.tsx`, se implementa el componente de estado de suscripción dentro de `<SidebarFooter>`, mostrando de forma sticky en la base del sidebar el plan activo (Free, Pro, Enterprise, etc.) y el botón de acción para actualizar plan (`/pages/billing/upgrade`), con comportamiento colapsable a icono con tooltip y auto-cierre en móvil.
    - **Reubicación del Botón Flotante de NovAi Copilot:** En `src/views/apps/investigator/shared/ai-copilot-sheet.tsx`, se elimina el desplazamiento artificial `right-[235px]` y se fija el trigger flotante en la esquina inferior derecha `fixed bottom-6 right-6 z-40 sm:bottom-8 sm:right-8` (y `bottom-4 right-4` en móviles), evitando colisiones visuales y centralizaciones indeseadas.
