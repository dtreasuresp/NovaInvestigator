...
-- =============================================================================
-- Fix 2026-08-22: consume_billing_entitlement_usage debe resolver límite desde
-- tenant_entitlements como get_billing_entitlement_usage (override → plan → tenant)
-- SODA §5.1 tenant-scoped, §5.2 auditoría, §5.3 idempotencia transaccional
-- =============================================================================
...
revoke all on function public.consume_billing_entitlement_usage(uuid, text) from public, anon, authenticated;
grant execute on function public.consume_billing_entitlement_usage(uuid, text) to authenticated, service_role;

comment on function public.consume_billing_entitlement_usage(uuid, text) is 'Fixed 2026-08-22: Aligns with get_billing_entitlement_usage — resolves limit via override→subscription→tenant_entitlements; missing=>0 denied, null=>unlimited, capped increment transaccional.';
