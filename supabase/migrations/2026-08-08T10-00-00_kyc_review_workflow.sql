...
create table if not exists public.kyc_requests (
  ...
  provider_reference text
    check (provider_reference is null or char_length(provider_reference) between 1 and 255),
  ...
  check (
    ...
    or (status in ('pending', 'under_review') and reviewed_at is null)
  )
);

comment on table public.kyc_requests is
  'KYC workflow metadata and decisions only. Raw identity documents are never stored here.';

comment on column public.kyc_requests.retention_until is
  'Minimum retention deadline for the verification result and metadata. Legal requirements may extend it.';

create index if not exists kyc_requests_status_submitted_idx
  on public.kyc_requests (status, submitted_at desc);

create index if not exists kyc_requests_user_created_idx
  on public.kyc_requests (user_id, created_at desc);

create unique index if not exists kyc_requests_one_open_per_user_idx
  on public.kyc_requests (user_id)
  where status in ('pending', 'under_review');

drop trigger if exists kyc_requests_set_updated_at on public.kyc_requests;
create trigger kyc_requests_set_updated_at
before update on public.kyc_requests
for each row execute function public.set_updated_at();

drop policy if exists profiles_select_platform_kyc on public.profiles;
create policy profiles_select_platform_kyc
on public.profiles
for select
to authenticated
using (
  public.has_platform_capability(auth.uid(), 'platform.kyc.read')
);

alter table public.kyc_requests enable row level security;

drop policy if exists kyc_requests_select_visible on public.kyc_requests;
create policy kyc_requests_select_visible
on public.kyc_requests
for select
to authenticated
using (
  user_id = auth.uid()
  or public.has_platform_capability(auth.uid(), 'platform.kyc.read')
);
...
revoke all on table public.kyc_requests from public, anon;
grant select, insert on table public.kyc_requests to authenticated;
