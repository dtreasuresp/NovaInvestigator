...
-- Platform-only identities are provisioned by
-- `provision_initial_super_admin` and intentionally do not need a
-- tenant-scoped profile, tenant, or workspace before they can sign in.
...
begin
  ...
  if not found then
    ...
    if exists (
      select 1
      from public.platform_memberships as platform_membership
      where platform_membership.user_id = p_user_id
