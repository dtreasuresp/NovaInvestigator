    ...
    <div className='w-full'>
      <div className='border-b'>
        <div className='flex min-h-17 flex-wrap items-center justify-between gap-3 px-4 py-3'>
          <span className='text-lg font-medium'>{t('userProfile.myProjects') || 'Lista de Proyectos'}</span>
          <div>
        ...
        <Table>
          <TableHeader>
            <TableRow className='h-17 border-t'>
              ...
              <TableHead className='text-muted-foreground'>{t('kanban.allProjects')}</TableHead>
              <TableHead className='text-muted-foreground'>{t('dashboards.responsible') || 'Líder'}</TableHead>
              <TableHead className='text-muted-foreground'>{t('pricingPage.featWorkTeams')}</TableHead>
              <TableHead className='text-muted-foreground'>{t('dashboards.progressIndicator')}</TableHead>
              <TableHead className='text-muted-foreground w-12 px-4 text-right'>{t('common.actions')}</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {projects.length === 0 ? (
                  ...
                  <TableCell className='px-4 text-right'>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align='end'>
                        <DropdownMenuGroup>
                          <DropdownMenuItem>{t('common.details') || 'Ver'}</DropdownMenuItem>
                          <DropdownMenuItem>{t('users.editRole')}</DropdownMenuItem>
                        </DropdownMenuGroup>
                      </DropdownMenuContent>
                    </DropdownMenu>
                  </TableCell>
      ...
      {totalProjects > 0 && (
        <div className='flex items-center justify-between gap-3 px-6 py-4 max-sm:flex-col md:max-lg:flex-col'>
          <p className='text-muted-foreground text-sm whitespace-nowrap' aria-live='polite'>
            {t('datatables.showingEntries', { from: showingFrom, to: showingTo, total: totalProjects })}
          </p>
