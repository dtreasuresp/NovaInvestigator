...
// `role` here is a tenant role key (string), not the legacy template
// `UserRole` union — the real admin member list filters by
// `roles.key` (system roles like 'owner'/'admin'/'analyst'/'viewer' or a
// tenant-defined custom role), which is not a fixed literal union.
// `plan` was dropped from the real admin member filters: plan/entitlement is
// a billing-domain concept (see doc/plans/PLAN_MAESTRO_SUPABASE_BILLING_ACCESS_2026-08-07.md
// section 12), out of scope here, and has no membership-level column in the
// access foundation schema to filter on.
...
// For column visibility. `plan`/`billing` removed: the real admin member
// table has no membership-level plan/billing columns (see UserFilters above).
export type UserColumnId = 'select' | 'user' | 'role' | 'status' | 'joinedDate' | 'actions'
