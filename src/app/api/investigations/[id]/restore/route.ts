// POST /api/investigations/:id/restore — restores an archived investigation; requires `version`.
import { NextResponse } from 'next/server'

import { idParamSchema, versionOnlyRequestSchema } from '@/lib/investigations/schema'
import { restoreInvestigation } from '@/lib/investigations/service'
...
export async function POST(request: Request, { params }: RouteContext) {
  try {
    ...
    const record = await restoreInvestigation(id, input)
